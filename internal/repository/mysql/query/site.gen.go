...
func newSite(db *gorm.DB, opts ...gen.DOOption) site {
	...
	_site.URL = field.NewString(tableName, "url")
	_site.CreatedAt = field.NewTime(tableName, "created_at")
	_site.UpdatedAt = field.NewTime(tableName, "updated_at")
	_site.IsUsed = field.NewInt64(tableName, "is_used")
	_site.Category = siteBelongsToCategory{
...
type site struct {
	...
	Thumb       field.String // 网站logo
	Description field.String // 网站描述
	URL         field.String // 网站地址
	CreatedAt   field.Time   // 创建时间
	UpdatedAt   field.Time   // 更新时间
	IsUsed      field.Int64  // 是否使用
	Category    siteBelongsToCategory
...
type siteBelongsToCategory struct {
	db *gorm.DB

	field.RelationField
}

func (a siteBelongsToCategory) Where(conds ...field.Expr) *siteBelongsToCategory {
	if len(conds) == 0 {
		return &a
	}
...
func (a siteBelongsToCategory) WithContext(ctx context.Context) *siteBelongsToCategory {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a siteBelongsToCategory) Session(session *gorm.Session) *siteBelongsToCategory {
	a.db = a.db.Session(session)
	return &a
}

func (a siteBelongsToCategory) Model(m *model.Site) *siteBelongsToCategoryTx {
	return &siteBelongsToCategoryTx{a.db.Model(m).Association(a.Name())}
}

type siteBelongsToCategoryTx struct{ tx *gorm.Association }

func (a siteBelongsToCategoryTx) Find() (result *model.Category, err error) {
	return result, a.tx.Find(&result)
}

func (a siteBelongsToCategoryTx) Append(values ...*model.Category) (err error) {
...
func (a siteBelongsToCategoryTx) Replace(values ...*model.Category) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteBelongsToCategoryTx) Delete(values ...*model.Category) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a siteBelongsToCategoryTx) Clear() error {
	return a.tx.Clear()
}

func (a siteBelongsToCategoryTx) Count() int64 {
	return a.tx.Count()
}
...
func (s siteDo) Exists(subquery interface{ UnderlyingDB() *gorm.DB }) ISiteDo {
