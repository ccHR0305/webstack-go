...
type modifyRequest struct {
	...
	Protocol            int64  `form:"protocol" binding:"required"`       // 执行方式 1:shell 2:http
	HttpMethod          int64  `form:"http_method"`                       // http 请求方式 1:get 2:post
	Timeout             int64  `form:"timeout" binding:"required"`        // 超时时间(单位:秒)
	RetryTimes          int64  `form:"retry_times" binding:"required"`    // 重试次数
	RetryInterval       int64  `form:"retry_interval" binding:"required"` // 重试间隔(单位:秒)
	NotifyStatus        int64  `form:"notify_status" binding:"required"`  // 执行结束是否通知 1:不通知 2:失败通知 3:结束通知 4:结果关键字匹配通知
	NotifyType          int64  `form:"notify_type"`                       // 通知类型 1:邮件 2:webhook
	...
	IsUsed              int64  `form:"is_used" binding:"required"`        // 是否启用 1:是  -1:否
}
